...
import { setLeadStatusAction, deleteLead } from "./actions";
...
// Кнопка перевода в статус. Текущий статус не показываем — переводить
// в него некуда.
...
export default async function AdminLeadsPage({
  ...
  const active = statusParam && isStatus(statusParam) ? statusParam : null;
  ...
  const tabs: { key: string; href: string; label: string; count: number }[] = [
    { key: "all", href: "/admin/leads", label: "Все", count: counts.all ?? 0 },
    ...LEAD_STATUSES.map((s) => ({
      key: s,
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {tab.label}
              <span className={isActive ? "ml-1.5 opacity-70" : "ml-1.5 text-[var(--color-gray-500)]"}>
                {tab.count}
              </span>
            </Link>
          );
            ...
            >
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
                ...
                <div className="flex shrink-0 flex-wrap items-center gap-x-4 gap-y-2 sm:justify-end">
                  ...
                  {/* Удаление только из «Спама»: заявка — персональные
              ...
              {/* Видно, какой канал не сработал. Telegram с российских
                  хостингов сейчас недоступен — это ожидаемо. */}
